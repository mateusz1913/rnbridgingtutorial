 ...
 */
#import "AppInfoPackageClassic.h"

// Each turbo module implements codegenerated spec protocol
@interface AppInfoModuleClassic () <NativeAppInfoModuleClassicSpec>
@end
#endif

// Declare the ObjC implementation for that native module class
@implementation AppInfoModuleClassic {
    AppInfoModuleClassicImpl *moduleImpl;
}

// Return the name of the module - it should match the name provided in JS specification
RCT_EXPORT_MODULE(AppInfoModuleClassic)

- (instancetype)init {
    self = [super init];
    if (self) {
        moduleImpl = [AppInfoModuleClassicImpl new];
    }
    return self;
}
...
{
    return std::make_shared<facebook::react::NativeAppInfoModuleClassicSpecJSI>(params);
}
#endif
