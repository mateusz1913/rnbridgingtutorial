---
sidebar_label: Package boilerplate
sidebar_position: 2
...
Each package that will be created in this tutorial needs to have some boilerplate. This section will showcase which files need to be created

:::info

Replace all `<package-name>` with your package name in kebab-case

Replace all `<PackageName>` with your package name in CamelCase

Replace all `<packagename>` with your package name in lowercase (without any spaces)

:::

#### `package.json`

Let's start with creating `<package-name>` directory containing package's code.
...
In this case, we want to code-generate the package with name `<PackageName>` with JS specs in `src` directory and `com.<packagename>` Android package name.

#### `<PackageName>.podspec`

Next, let's create `<PackageName>.podspec`, the "equivalent" of `package.json`, but for CocoaPods packages:

```ruby title="<PackageName>.podspec"
