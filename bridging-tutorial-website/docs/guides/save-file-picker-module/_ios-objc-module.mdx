...
Before declaring the implementation block, we have to import header file for `SaveFilePickerModuleImpl` Objective-C class,
...
We can use `AppInfoModuleImpl` Objective-C class, because it's imported from `AppInfoModuleImpl.h`.
