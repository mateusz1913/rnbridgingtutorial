...
Before declaring the implementation block, we have to import header file for `SaveFilePickerModuleImpl` module-implementation class,
...
We can use `AppInfoModuleImpl` ObjC class, because it's imported from `AppInfoModuleImpl.h`.
