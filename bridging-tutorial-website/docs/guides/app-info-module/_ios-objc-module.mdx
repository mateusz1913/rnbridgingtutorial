...
Before declaring the implementation block, we have to import header file for `AppInfoModuleImpl` class,
...
First, we declare private variable on `AppInfoModule` which will be initialized in the `- init` method. We can use `AppInfoModuleImpl` Objective-C class, because it's imported from `AppInfoModuleImpl.h`.
